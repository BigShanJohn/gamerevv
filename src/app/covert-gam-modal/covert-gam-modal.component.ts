...
@Component({
  selector: 'app-covert-gam-modal',
  templateUrl: './covert-gam-modal.component.html',
  styleUrls: ['./covert-gam-modal.component.css']
})

export class CovertGamModalComponent implements OnInit {
  @Input() balance: any;
  @Input() fee: any;
  ...
  amountValidator() {
    ...
    if (parseFloat(this.amount) < 3) {
      return this.amountError = "The minimum amount of GAV you can swap is 3";
    }
  ...
  submit() {
    ...
    let url = `${Common.API_URL}convert-to-gam`;
  ...
  get receivable() {
    return (parseFloat(this.amount.replace(/,/g, '')) - parseFloat(this.fee.general)) * 1000;
  }
